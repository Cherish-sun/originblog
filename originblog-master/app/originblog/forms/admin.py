...
class MetaUserForm(FlaskForm):
    ...
    email_confirmed = BooleanField('邮箱确认')
    role = SelectField('角色', choices=ROLES)
    name = StringField('姓名', validators=[Length(1, 128)])
    bio = StringField('签名', validators=[Optional(), Length(0, 200)])
    ...
    active = BooleanField('操作', default=True)
...
class PostForm(FlaskForm):
    ...
    weight = IntegerField('权重', default=10)
    raw_content = TextAreaField('内容', validators=[DataRequired()])
    abstract = TextAreaField('摘要', validators=[Optional(), Length(0, 255)])
    category = StringField('分类', validators=[Optional(), Length(0, 64)])
    tags = StringField('标签（用空格分隔）', validators=[Optional(), Length(0, 64)])
